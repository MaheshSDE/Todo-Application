...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  response.send(ans(responseResult));
});
...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    const requestQuery = `
        ...
        `;
    const resultResponse = await db.all(requestQuery);
    response.send(resultResponse.map((eachItem) => ans(eachItem)));
  } else {
    response.status(400);
